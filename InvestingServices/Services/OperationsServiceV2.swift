...
public protocol OperationsServing {
    func loadOperations(for account: BrokerAccount, progress: @escaping (LoadingProgress) -> ()) -> AnyPublisher<[OperationV2], Error>
}
...
extension OperationsServiceV2: OperationsServing {
    func loadOperations(
        for account: BrokerAccount,
        progress: @escaping (LoadingProgress) -> ()
    ) -> AnyPublisher<[OperationV2], Error> {
        ...
        return Publishers.Sequence(sequence: publishers)
            .flatMap(maxPublishers: .max(1)) { publisher in
                requestNumber += 1
                progress(
                    LoadingProgress(current: requestNumber, all: publishers.count)
                )

                return publisher
                    .delay(for: APIConstants.requestDelay, scheduler: DispatchQueue.global())
            }
